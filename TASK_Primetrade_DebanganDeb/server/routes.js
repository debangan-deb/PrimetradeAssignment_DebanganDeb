...
r.post("/register", async (req, res) => {
  ...
  await pool.query("INSERT INTO users SET ?", { name, email, password: password });
  res.send("OK");
});
...
r.post("/login", async (req, res) => {
  try {
    ...
    // DIRECT (INSECURE) COMPARISON - local testing only
    const stored = rows[0].password; // plaintext stored in DB
    if (password !== stored) {
...
// Added signup route to ensure password hashing (if original route missing)
router.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    // Replace following with your User model save logic
    // Example using Mongoose:
    // const user = await User.create({ name, email, password: hashedPassword });
    return res.status(201).json({ message: 'Signup route added: please integrate with your User model', demo: { name, email } });
  } catch (err) {
    console.error(err);
...
// Added login route with bcrypt compare (fallback). Integrate with your User model.
router.post('/api/auth/login', async (req, res) => {
  try {
    const { email, password } = req.body;
    if (!email || !password) return res.status(400).json({ message: 'Missing email or password' });
    // Replace the following with real user lookup:
    // const user = await User.findOne({ email });
    // if (!user) return res.status(401).json({ message: 'Invalid credentials' });
    // const match = await bcrypt.compare(password, user.password);
    // if (!match) return res.status(401).json({ message: 'Invalid credentials' });
    return res.status(200).json({ message: 'Login route added: integrate with your User model' });
  } catch (err) {
    console.error(err);
